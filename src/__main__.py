...
group.add_argument("-b", "--binary", type=snarl_vcf_parser.check_format_pheno, help="Path to the binary group file (.txt or .tsv)")
...
if args.binary:
    ...
    snarl_vcf_parser.check_mathing(merged_dict, list_samples, "phenotype")
...
"""
Usage example:
    python3 src/__main__.py -p ../snarl_data/fly.pg -d ../snarl_data/fly.dist -v ../droso_data/pangenome.dm6.vcf \
    -r ../droso_data/fly.normalized.vg_deconstruct.vcf -q ../droso_data/pangenome_phenotype.tsv -o output

Usage test:
    python3 src/__main__.py -p test/simulation/pg.full.pg -d test/simulation/pg.dist -v test/simulation/merged_output.vcf.gz \
